  ...
  return (
    <div className="patient-list">
      <div className="pl-header">
        <h2>Patient Queue</h2>
        <span className="pl-count">{patients.length} Active</span>
      </div>
      ...
      <div className="st-dots">
        {sorted.map((log, i) => (
          <div key={i} className="st-dot-wrapper" title={`${log.date}: ${log.severity} — ${log.reason || 'OK'}`}>
            <div className="st-dot" style={{ backgroundColor: SEVERITY_COLORS[log.severity] || '#94a3b8' }} />
...
function ChatHistoryTab({ logs }) {
  if (!logs || logs.length === 0) return <div className="conv-empty">No chat history available</div>
  const logsWithConvo = logs.filter((l) => l.raw_response)
  if (logsWithConvo.length === 0) return <div className="conv-empty">No chat history recorded yet</div>
      ...
      {logsWithConvo.map((log, i) => {
        ...
        return (
          <div key={i} className="chat-history-day">
            ...
            {messages ? <ChatBubbles messages={messages} /> : <div className="conv-text-plain">{log.raw_response}</div>}
            {log.reason && (
              <div className="chd-reason"><strong>Assessment:</strong> {log.reason}</div>
            )}
          </div>
        )
      })}
...
function PatientDetail({ patientId }) {
  ...
  useEffect(() => {
    if (!patientId) return
    fetch(`${API}/patients/${patientId}`).then((r) => r.json()).then(setData)
    fetch(`${API}/patients/${patientId}/medicines`).then((r) => r.json()).then(setMedicines)
    setActiveTab('care')
  }, [patientId])
    ...
    <div className="patient-detail">
      <div className="pd-header">
        ...
        <div
          ...
          style={{
            backgroundColor: data.risk_level === 'high' ? '#fecaca' : data.risk_level === 'moderate' ? '#fef3c7' : '#dcfce7',
            color: data.risk_level === 'high' ? '#dc2626' : data.risk_level === 'moderate' ? '#d97706' : '#16a34a',
          }}
        >
      ...
      <div className="pd-tabs">
        {tabs.map((tab) => (
          <span key={tab.id} className={`pd-tab ${activeTab === tab.id ? 'active' : ''}`} onClick={() => setActiveTab(tab.id)}>
            {tab.icon} {tab.label}
          </span>
        ))}
      </div>
      ...
      {activeTab === 'chat' && <ChatHistoryTab logs={data.daily_logs} />}
      {activeTab === 'vitals' && <VitalsTab data={data} medicines={medicines} />}
    </div>
  )
}
    ...
    <div className="agent-feed">
      ...
      <div className="af-items">
        {allEvents.slice(0, 100).map((event, i) => (
          <div key={i} className="af-item" style={{ borderLeftColor: AGENT_COLORS[event.agent] || '#94a3b8' }}>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetchPatients()
    fetch(`${API}/events?limit=50`).then((r) => r.json()).then(setStoredEvents).catch(console.error)
    ...
    <div className="dashboard-page">
      ...
      <div className="dash-toolbar">
        <div className="dash-toolbar-left">
          <h2>🌸 Dashboard</h2>
          <span className="dash-patient-count">{patients.length} patients</span>
        </div>
        <div className="dash-toolbar-right">
          ...
          <div className="mobile-panel-switch">
            <button className={mobilePanel === 'list' ? 'active' : ''} onClick={() => setMobilePanel('list')}>Queue</button>
            <button className={mobilePanel === 'detail' ? 'active' : ''} onClick={() => setMobilePanel('detail')}>Detail</button>
            <button className={mobilePanel === 'feed' ? 'active' : ''} onClick={() => setMobilePanel('feed')}>Feed</button>
          </div>
        </div>
      </div>

      <div className="main-layout">
        <aside className={`sidebar-left ${mobilePanel === 'list' ? 'mobile-show' : 'mobile-hide'}`}>
          <PatientList patients={patients} selectedId={selectedId} onSelect={handleSelectPatient} />
        </aside>

        <main className={`center-panel ${mobilePanel === 'detail' ? 'mobile-show' : 'mobile-hide'}`}>
          <PatientDetail patientId={selectedId} />
        </main>

        <aside className={`sidebar-right ${mobilePanel === 'feed' ? 'mobile-show' : 'mobile-hide'}`}>
